...
Clearance, a default BlueLightSpecial installation includes:

	* optional support for authenticating with Facebook Connect, using
	...
	* use of {MadMimi}[http://madmimi.com] for sending notification emails
	
	* {delayed_job}[http://github.com/tobi/delayed_job] for sending emails
		in the background
...
After installing the BlueLightSpecial gem, create a new Rails app and run
...
Then, run the delayed_job migration generator and migrate your database:

	script/generate delayed_job
	rake db:migrate
	
You'll also want to update BlueLightSpecial's configuration options
in config/initializers/blue_light_special.rb.

== Tests

BlueLightSpecial can optionally generate integration tests that cover all
...
The final BlueLightSpecial generator builds a very simple administration
